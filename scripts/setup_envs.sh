...
create_or_update_env_from_yaml () {
  ...
  if conda env list | awk '{print $1}' | grep -qx "$ENVNAME"; then
    ...
    conda env update -n "$ENVNAME" -f "$YAMLFILE" --prune
  else
    echo "[INFO] Creating env '$ENVNAME' from $YAMLFILE"
    conda env create -n "$ENVNAME" -f "$YAMLFILE"
  fi
}
...
print_versions () {
  ...
  conda run -n "$ENVNAME" python -c "import sys; print('python', sys.version.split()[0])"
  conda run -n "$ENVNAME" python -c "import torch; print('torch', torch.__version__, '| cuda', torch.version.cuda); print('cuda_available', torch.cuda.is_available())" 2>/dev/null || true

  if [[ "$TYPE" == "core" ]]; then
    conda run -n "$ENVNAME" python -c "import schnetpack; print('schnetpack', schnetpack.__version__)" 2>/dev/null || true
  elif [[ "$TYPE" == "nequip" ]]; then
    conda run -n "$ENVNAME" python -c "import nequip; print('nequip', nequip.__version__)" 2>/dev/null || true
  elif [[ "$TYPE" == "mace" ]]; then
    conda run -n "$ENVNAME" python -c "import mace; print('mace', mace.__version__)" 2>/dev/null || true
  fi
